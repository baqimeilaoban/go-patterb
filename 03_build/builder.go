...
func (b *ResourcePoolConfigBuilder) SetMinIdle(minIdle int) error {
	if minIdle < 0 {
		return fmt.Errorf("min total cannot < 0, input: %d", minIdle)
	}
	b.minIdle = minIdle
	return nil
}
...
func (b *ResourcePoolConfigBuilder) Build() (*ResourcePoolConfig, error) {
	...
	if b.maxTotal < b.maxIdle {
		return nil, fmt.Errorf("max total(%d) cannot < maxidle(%d)", b.maxTotal, b.maxIdle)
	}

	if b.minIdle > b.maxIdle {
		return nil, fmt.Errorf("maxidle(%d) cannot < minidle(%d)", b.minIdle, b.minIdle)
	}
